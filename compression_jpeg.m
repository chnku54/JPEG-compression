...
for block=1:4096
    Im_DCT(block,1:8,1:8) = reshape(dct2(B(block,:,:)),8,8);
end
...
for block=1:4096
    C1(block,1:8, 1:8) = round(Im_DCT(block,1:8, 1:8)./Q2);
end
...
for block=1:4096
    DC_Q(block,1:8, 1:8) = DC1(block,1:8, 1:8).*Q2;
end
...
for block=1:4096
    DB(block,1:8,1:8) = reshape(idct2(DC_Q(block,:,:)),8,8);
end
